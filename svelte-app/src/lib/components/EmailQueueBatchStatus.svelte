<script lang="ts">
    ...
    const PROCESSOR_API = '/api/email-queue-batch';
    ...
    async function handleStartProcessor() {
        try {
            ...
            console.error('매일 발송 배치 시작 중 오류:', error);
        }
    }
    ...
    async function handleStopProcessor() {
        try {
            ...
            console.error('매일 발송 배치 중지 중 오류:', error);
        }
    }
    ...
    async function loadProcessorStatus() {
        try {
            ...
            console.error('매일 발송 배치 상태를 불러오는 중 오류 발생:', error);
        }
    }
...
<Card class="rounded-lg p-6 shadow-lg dark:text-white" size="xl">
    <div class="mb-4 flex items-center justify-between">
        <h3 class="text-lg font-semibold">매일 발송 배치</h3>
    </div>
